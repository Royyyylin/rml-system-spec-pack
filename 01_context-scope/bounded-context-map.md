...
下表 6 條為 system-level authority boundary rule, 規定每個 actor 在權威邊界內可做與不可做的事。不可被任一 use case 跨越。**L3 source-level refactor 2026-04-27** (per ADR-013): 廢 `RML-AUT-NNN` opaque ID schema, 改 boundary-name canonical primary key + cross-ref by `file.md#section-anchor`。

| Boundary Name | Boundary Rule | Cross-Ref |
| :--- | :--- | :--- |
| **Operator-Limited-Authority** | Operator 可查看與執行已授權的低風險操作，但不得單獨定義 canonical identity、assignment truth 或 system policy。 | [stakeholders.md#roles](../00_introduction-goals/stakeholders.md#roles) |
| **Engineer-Diagnostic-Authority** | Engineer 可執行高權限診斷與維護流程，但不得繞過 Central authority 或直接改寫 repo SSOT。 | [stakeholders.md#roles](../00_introduction-goals/stakeholders.md#roles) |
| **Firmware-Runtime-Truth-Ownership** | Gateway / Edge Nodes 擁有 runtime measurement、local QoS、local failover 觸發與 radio behavior 的第一手 truth；Central 與 App 不得假裝擁有這層 truth。 | [#gateway--edge-nodes](#gateway--edge-nodes), [`Runtime-Observation-Authority`](../02_solution-strategy/capability-map.md) |
| **Central-Global-Truth-Authority** | Central System 擁有 global truth / system-of-record authority；App、Firmware、Conductor 不得自行重定 canonical identity、assignment 或 auth truth。 | [#central-system](#central-system), [`Canonical-Identity-Authority`](../02_solution-strategy/capability-map.md) |
| **Conductor-Planning-Only** | Conductor / AI orchestration layer 只管理規劃、派工、驗收、handoff 與治理，不直接介入 real-time control loop，也不覆寫 runtime truth。 | [#conductor-ai-orchestration-layer](#conductor-ai-orchestration-layer), [`AI-Orchestration-Non-Authority`](../00_introduction-goals/system-intent.md#engineering-invariants) |
| **Orchestration-Vs-Repo-Truth-Layer** | project-level orchestration truth 與 repo-level technical truth 必須分層；`--base-dir` 只承接 cross-repo control docs，不承接 repo-level technical SSOT。 | [`Base-Dir-Cross-Repo-Only`](../00_introduction-goals/system-intent.md#engineering-invariants), [`Two-Layer-Operating-Model`](../00_introduction-goals/system-intent.md#strategic-goals) |

## Scope

L3 refactor 2026-04-27 (per ADR-013): 廢 `RML-SCP-NNN` opaque ID schema, 改 scope-type canonical name。

| Scope Type | Statement |
| :--- | :--- |
| **In Scope — System UI / Telemetry** | device list/detail、telemetry 顯示、roster 顯示、alias precedence、assignment reconciliation、command feedback。 |
| **Out of Scope — Deferred Features** | OTA、歷史分析、desktop 專屬 UI 規格、本輪未提供來源的新 Central API。 |
| **In Scope — Cross-Repo Governance** | cross-repo orchestration、queue / gate governance、handoff、evidence index、project-level acceptance model。 |
| **Out of Scope — AI Authority Limits** | 讓 AI orchestration layer 直接成為 runtime control authority，或在 `--base-dir` 複製 repo-level technical SSOT。 |
...
**Responsibility**: 產生 runtime truth，執行 QoS、uplink、HA、local coordination 與裝置側行為。GW 擁有 local QoS 排程、local failover 觸發與 radio behavior 的第一手 truth；ED 負責 runtime measurement 與 device-side behavior。Central 與 App 不得假裝擁有這層 truth（per `Firmware-Runtime-Truth-Ownership` boundary, see [#authority-boundaries](#authority-boundaries)）。
...
**Responsibility**: 維護 canonical identity、assignment、metadata、auth、audit、sync 與 global truth。App、Firmware、Conductor 不得自行重定 canonical identity、assignment 或 auth truth（per `Central-Global-Truth-Authority` boundary, see [#authority-boundaries](#authority-boundaries)）。
...
**Responsibility**: 管理 planning、dispatch、handoff、queue、evidence 與 cross-repo governance。不直接介入 real-time control loop，不覆寫 runtime truth，不取代 Central 或 Firmware 的權威邊界（per `Conductor-Planning-Only` boundary, see [#authority-boundaries](#authority-boundaries)）。`--base-dir` 只承接 cross-repo formal control docs，不承接 repo-level technical SSOT（per `Orchestration-Vs-Repo-Truth-Layer` boundary）。
...
- Evans relationships are annotations on existing authority boundaries (see [#authority-boundaries](#authority-boundaries) 6 boundary names); they do not change authority ownership.
